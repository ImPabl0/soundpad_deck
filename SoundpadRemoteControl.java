...
public class SoundpadRemoteControl {
     ...
     * @return true on success
     */
    public boolean playRandomSound(final boolean renderLine, final boolean captureLine) {
        return isSuccessPrintResponse(
                sendRequestNoException("DoPlayRandomSound(" + renderLine + ", " + captureLine + ")"));
    }
     ...
     *
     * @param categoryIndex
     *            set to -1 to play a random sound from the currently selected
     *            category or use {@link #getCategories(boolean, boolean)} to
     ...
     * @return true on success
     */
    public boolean playRandomSoundFromCategory(final int categoryIndex, final boolean renderLine,
                                               final boolean captureLine) {
        return isSuccessPrintResponse(sendRequestNoException(
    ...
    /**
     * @return true if the client is using the trial version or false if it is
     *         the full version.
     */
    public boolean isTrial() {
        String response = sendRequestNoException("IsTrial()");
        if (response.isEmpty()) {
            printOfflineError();
        } else {
            try {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        SoundpadRemoteControl control = new SoundpadRemoteControl();
        control.init();
        if (control.isAlive()) {
            System.out.println("Soundpad is alive!");
            System.out.println("Soundpad version: " + control.getVersion());
            System.out.println("Remote control version: " + control.getRemoteControlVersion());
            System.out.println("Is trial version: " + control.isTrial());
            System.out.println("List sounds: " + control.getSoundlist());
            control.playSound(7, false, true);
        } else {
            System.out.println("Soundpad is not alive.");
        }
}}